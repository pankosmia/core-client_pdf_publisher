...
const getObsNotes = async (notesPath, notesRef, i18nRef) => {
  let obsResponse = await getText(
    `/api/burrito/ingredient/raw/${notesPath}?ipath=${"OBS.tsv"}`,
  );
  if (obsResponse.ok) {
    return obsResponse.text
      .split("\n")
      .map((l) => l.trim())
        ...
        if (imgRepoResponse.ok) {
          ...
          if (section.content.notes) {
            ...
            while (RegExp(/%%%%NOTES%%%%/).test(markdown)) {
              ...
              markdown = markdown.replace(
                "%%%%NOTES%%%%",
                await getObsNotes(section.content.notes, noteParaRef, i18nRef),
              );
              noteParaN++;
            }
          }
